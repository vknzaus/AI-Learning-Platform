...
const getDifficultyColor = (difficulty: string) => {
  switch (difficulty) {
    case "Beginner":
      return "bg-slate-700 text-teal-300 border-slate-600";
    case "Intermediate":
      return "bg-slate-700 text-blue-300 border-slate-600";
    case "Advanced":
      return "bg-slate-700 text-purple-300 border-slate-600";
    default:
...
const getCardColor = (color: string) => {
  switch (color) {
    case "green":
      return "border-slate-600 hover:border-teal-500 bg-slate-800";
    case "blue":
      return "border-slate-600 hover:border-blue-500 bg-slate-800";
    case "purple":
      return "border-slate-600 hover:border-purple-500 bg-slate-800";
    case "yellow":
      return "border-slate-600 hover:border-yellow-500 bg-slate-800";
    case "red":
      return "border-slate-600 hover:border-red-500 bg-slate-800";
    default:
      return "border-slate-600 hover:border-gray-500 bg-slate-800";
  }
};
...
export const Practice: React.FC<PracticeProps> = ({ className = "" }) => {
  const handleStartPractice = (exerciseId: string) => {
    ...
    alert(
      `🎯 Starting practice exercise ${exerciseId}!\n\nThis feature will be implemented soon.`
    );
  };
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto mb-8">
        <div className="bg-slate-800 border-2 border-teal-500 rounded-2xl p-6 text-center">
          <div className="text-teal-400 font-black text-3xl mb-2">
            {completedCount}
          </div>
        ...
        <div className="bg-slate-800 border-2 border-purple-500 rounded-2xl p-6 text-center">
          <div className="text-purple-400 font-black text-3xl mb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {practiceExercises.map((exercise) => (
            <div
              ...
              `}
              onClick={() =>
                !exercise.completed && handleStartPractice(exercise.id)
              }
            >
              <div className="flex items-start justify-between mb-4">
                <div className="text-4xl">{exercise.icon}</div>
                <div
                  className={`px-3 py-1 rounded-full text-sm font-bold border ${getDifficultyColor(
                    exercise.difficulty
                  )}`}
                >
                  {exercise.difficulty}
