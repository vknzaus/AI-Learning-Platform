...
export const TopicCard = ({ topic, onStartLearning }: TopicCardProps) => {
  const getTopicIcon = (name: string) => {
    if (name.toLowerCase().includes('ai') || name.toLowerCase().includes('artificial')) return '🤖';
    if (name.toLowerCase().includes('machine learning') || name.toLowerCase().includes('ml')) return '🧠';
  ...
  const getTopicColor = (name: string) => {
    if (name.toLowerCase().includes('ai') || name.toLowerCase().includes('artificial')) return 'blue';
    if (name.toLowerCase().includes('machine learning') || name.toLowerCase().includes('ml')) return 'green';
  ...
  return (
    <div className={`
      bg-white border-2 ${borderColors[color]} 
    ...
    `}>
      ...
      <div className={`bg-gradient-to-br ${gradients[color]} h-32 flex items-center justify-center relative overflow-hidden`}>
        {/* Decorative circles */}
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-3">
            <div className="bg-duogreen-100 text-duogreen-800 border border-duogreen-200 font-bold px-3 py-1 rounded-full text-xs">
              🚀 Beginner Friendly
            </div>
            <div className={`text-sm font-bold px-3 py-1 rounded-full bg-${color === 'blue' ? 'duoblue' : color === 'green' ? 'duogreen' : color === 'purple' ? 'duopurple' : color === 'yellow' ? 'duoyellow' : 'duored'}-50 text-${color === 'blue' ? 'duoblue' : color === 'green' ? 'duogreen' : color === 'purple' ? 'duopurple' : color === 'yellow' ? 'duoyellow' : 'duored'}-700`}>
              📚 {topic.lessons.length} lesson{topic.lessons.length !== 1 ? 's' : ''}
            </div>
          </div>
          <h3 className="text-xl font-black text-duogray-800 mb-2 group-hover:text-duogreen-600 transition-colors line-clamp-2">
            {topic.name}
          </h3>
        </div>

        {/* Description */}
        <p className="text-duogray-600 text-sm mb-6 line-clamp-3 leading-relaxed font-medium">
          {topic.description || '🎯 Master the fundamentals with fun, bite-sized lessons! Build amazing projects and unlock your potential with our expert-crafted curriculum. ✨'}
        </p>

        {/* Progress Indicator */}
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs font-bold text-duogray-600">Ready to start!</span>
            <span className="text-xs font-bold text-duogreen-600">0% Complete</span>
          </div>
          <div className="w-full bg-duogray-200 rounded-full h-2 overflow-hidden">
            <div className="bg-gradient-to-r from-duogreen-400 to-duoblue-400 h-2 rounded-full w-0 group-hover:w-1/4 transition-all duration-1000"></div>
          </div>
        </div>
        ...
        {/* Fun encouragement */}
        <p className="text-center text-xs text-duogray-500 mt-3 font-medium">
